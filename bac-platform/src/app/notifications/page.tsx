...
type Notification = { id: string; title: string; message: string | null; question_id: string | null; is_read: boolean; created_at: string; type: "new_question" | "question_answered" | "announcement" | string };
...
export default function NotificationsPage() {
  ...
  const load = async () => { if (!user) return; const { data } = await supabase.from("notifications").select("id, title, message, question_id, is_read, created_at, type").eq("user_id", user.id).in("type", ["new_question", "question_answered", "announcement"]).order("created_at", { ascending: false }); setItems((data as Notification[] | null) ?? []); };
  useEffect(() => { load(); }, [user]);
